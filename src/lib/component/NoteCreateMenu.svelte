...
<form
	...
	use:enhance={() => {
		return async ({ result }) => {
			state.addNote(result.data.noteId);
			state.hideOverlay();
		};
	}}
>
	...
	<div class="input-container">
		<input name="name" type="text" placeholder="Name your note..." bind:value={data.input.name} />
		<input name="date" type="date" bind:value={data.input.date} />
		<textarea name="text" placeholder="Your note content..." bind:value={data.input.text}
		></textarea>
	</div>
	<label>
		Background color
	...
	<button class="btn__create-note">Create Note</button>
</form>
